...
import { getFederatedApps, type FederatedApp } from "~/models/app.server";
import { type SimulationNodeDatum } from "d3";
import Toolbar from "~/components/toolbar";
import useWindow from "~/hooks/useWindow";

export const loader: LoaderFunction = async (args): Promise<FederatedApp[]> => {
  return await getFederatedApps();
};
    ...
    function ticked() {
      const g = svg
        ...
        .on("click", (e, node) => {
          ...
          navigate(`/app?host=${node.host}`);
        });

      g.append("circle").style("fill", "#3260a8").attr("r", defaultRadius);
